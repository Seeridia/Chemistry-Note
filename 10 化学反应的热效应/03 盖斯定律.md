...
但下列两个反应的 $\Delta H$ 可以直接测得：
 ...
 $$\ce{C(s) + O2(g) = CO2(g)}\qquad①  \qquad \Delta H_1 = -393.5 \text{ kJ/mol}$$
...
① 式与 ③ 式相加后，消去 $\ce{CO2(g)}$ 和 $\ce{1/2O2(g)}$ ,方程式变化为：

$\ce{C(s) +1/2O2(g)=CO(g)}\qquad\Delta H=\Delta H_1-\Delta H_2=-110.5\text{ KJ/mol}$